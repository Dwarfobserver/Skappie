...
namespace sk {
	...
	public:
		using iterator = pointer_iterator<slot_map<T, Allocator>, T, sizeof(data_t)>;
		using const_iterator = const_pointer_iterator<slot_map<T, Allocator>, T, sizeof(data_t)>;
		using reverse_iterator = reverse_pointer_iterator<slot_map<T, Allocator>, T, sizeof(data_t)>;
		using const_reverse_iterator = const_reverse_pointer_iterator<slot_map<T, Allocator>, T, sizeof(data_t)>;
	...
	template <class T, template <class> class Allocator>
	int slot_map<T, Allocator>::size() const noexcept {
		return size_;
	}

	template <class T, template <class> class Allocator>
	T& slot_map<T, Allocator>::operator[](key k) noexcept {
	...
	template <class T, template <class> class Allocator>
	T const& slot_map<T, Allocator>::operator[](key k) const noexcept {
		key k2 = indices[k.pos()];
		assert(k2.gen() == k.gen() && "The key is not valid anymore (the object has been deleted");
		return objects[k2.pos()].val;
	}
	...
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::iterator slot_map<T, Allocator>::begin() noexcept {
		return iterator(objects.data());
	}
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::iterator slot_map<T, Allocator>::end() noexcept {
		return iterator(objects.data() + size_);
	}
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::const_iterator slot_map<T, Allocator>::cbegin() const noexcept {
		return const_iterator(objects.data());
	}
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::const_iterator slot_map<T, Allocator>::cend() const noexcept {
		return const_iterator(objects.data() + size_);
	}
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::reverse_iterator slot_map<T, Allocator>::rbegin() noexcept {
		return reverse_iterator(objects.back());
	}
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::reverse_iterator slot_map<T, Allocator>::rend() noexcept {
		return reverse_iterator(objects.data() - 1);
	}
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::const_reverse_iterator slot_map<T, Allocator>::crbegin() const noexcept {
		return const_reverse_iterator(objects.back());
	}
	template <class T, template <class> class Allocator>
	typename slot_map<T, Allocator>::const_reverse_iterator slot_map<T, Allocator>::crend() const noexcept {
